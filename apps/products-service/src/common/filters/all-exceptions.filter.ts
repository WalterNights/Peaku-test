...
import { DuplicateSkuError, ProductNotFoundError } from '../../domain/errors';
...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  private resolveStatus(exception: unknown): number {
    ...
    if (exception instanceof DuplicateSkuError) {
  ...
  private resolveBody(exception: unknown): { message: string; error: string; details?: ErrorDetail[] } {
    if (exception instanceof ProductNotFoundError || exception instanceof DuplicateSkuError) {
