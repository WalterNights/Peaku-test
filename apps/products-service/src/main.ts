...
async function bootstrap(): Promise<void> {
  ...
  const port = config.get<number>('PRODUCTS_PORT') ?? config.get<number>('PORT') ?? 3002;
  ...
  if (config.get<string>('SWAGGER_ENABLED') === 'true') {
    const swaggerConfig = new DocumentBuilder()
      .setTitle('Products Service · PeaKu')
      .setDescription('Catálogo de productos: CRUD con paginación, soft-delete y autorización por rol.')
      .setVersion('1.0.0')
      .addBearerAuth(
      ...
      .addTag('products', 'CRUD de productos del catálogo')
    ...
    SwaggerModule.setup('api/docs', app, document, {
      ...
      customSiteTitle: 'PeaKu · Products API',
    });
  }

  await app.listen(port, '0.0.0.0');
  // eslint-disable-next-line no-console
  console.log(`[products-service] listening on http://localhost:${port} (docs: /api/docs)`);
}
